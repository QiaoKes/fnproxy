...
func RegisterAPIs(server interface {
	Register(string, common.InterceptorFunc)
}) {
	// 用户相关API
	server.Register("/api/users", handleUsers)
	server.Register("/api/users/*", handleUserDetail)

	// 系统信息API
	server.Register("/api/system/info", handleSystemInfo)
	server.Register("/api/system/status", handleSystemStatus)

	// 代理管理API
	server.Register("/api/proxy/stats", handleProxyStats)
	server.Register("/api/proxy/config", handleProxyConfig)

	// 文件上传API
	server.Register("/api/upload", handleFileUpload)

	// 认证API
	server.Register("/api/auth/login", handleLogin)
	server.Register("/api/auth/logout", handleLogout)

	// 数据查询API
	server.Register("/api/data/search", handleDataSearch)

	// WebSocket API示例
	server.Register("/api/ws/connect", handleWebSocket)
}
